...
class buran_seoModule
{
	...
	function ob_end($template)
	{
		...
		$global = '_global.txt';
		$folder = $this->droot.'/_buran/seoModule/b/';
		...
		if ($http_code == 404 && $this->seotext) {
			...
			if ($fh) {
				while ( ! feof($fh)) {
					$template_file .= fread($fh, 1024*32);
				}
				fclose($fh);
			} else {
				$this->log('[30]');
		...
		if ($http_code == 200 && $this->seotext_tpl && $tags2) {
			...
			if (is_array($headers)) {
				...
				if (
					! $cookie &&
					time() - $this->filetime($folder.$global) > 60*5
				) {
					$file = $global;
				}
			}
			...
			if ($fh) {
				$res = fwrite($fh, $template_orig);
				fclose($fh);
			}
			if ($res === false) $this->log('[31]');
		}
