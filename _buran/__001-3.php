...
if( $act == 'antivirus' )
{
	...
	if( $r_kws && @mysql_num_rows( $r_kws ) > 0 )
	{
		...
		if( $fileslist )
		{
			//print '<pre>';print_r( $fileslist );print '</pre>';
			foreach( $fileslist AS $key => $row )
			{
				...
				if( $row[ 'files' ] )
				{
					...
					{
						$tmp .= '<a class="'. $row2[ 'type' ] .'" target="_blank" href="?act=printfile&w='. $_GET[ 'w' ] .'&dir='. $dir .'&file='. $key . $key2 .'" title="'. $key2 .'">'. $row2[ 'id' ] .'</a>';
					}
				}
					...
					if( $tmp_filesize > 0 )
					{
						...
						for( $kk=0; $kk < @mysql_num_rows( $r_kws ); $kk++ )
						{
												...
												if( @mysql_result( $r_kws, $kk, 'size' ) == 0 || ( @mysql_result( $r_kws, $kk, 'size' ) > 0 && @mysql_result( $r_kws, $kk, 'size' ) == $tmp_filesize ) )
												{
													...
													if( $ignoreignore || ( $rr && mysql_num_rows( $rr ) == 0 ) )
													{
														...
														if( ! $signatureinfoflag[ @mysql_result( $r_kws, $kk, 'id' ) ] )
														{
															...
															$signatureinfo .= '<div><pre>'. @mysql_result( $r_kws, $kk, 'id' ) .'. '. @mysql_result( $r_kws, $kk, 'name' ) .' | '. @mysql_result( $r_kws, $kk, 'kw_1' ) .' | '. @mysql_result( $r_kws, $kk, 'kw_2' ) .' | '. @mysql_result( $r_kws, $kk, 'kw_3' ) .' | '. @mysql_result( $r_kws, $kk, 'kw_4' ) .'</pre></div>';
														}
													}
												}
											}
							...
							if( $tmp2 == 'yea' )
							{
								...
								$fileslist[ $folder ][ 'files' ][ $file ]= array(
									'id' => @mysql_result( $r_kws, $kk, 'id' ),
									'type' => @mysql_result( $r_kws, $kk, 'type' )
								);
							}
