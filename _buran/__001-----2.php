...
if( $act == 'editfile' && $file != '' )
{
	...
	if( isset( $_POST[ 'save' ] ) )
	{
		...
		if( isset( $_GET[ 'cp1251' ] ) ) $content= iconv( 'UTF-8', 'CP1251//TRANSLIT//IGNORE', $content );
		$fp= @fopen( $path, 'wb' );
		if( $fp )
		{
			@fwrite( $fp, $content );
	...
	if( $fp )
	{
		...
		if( $content )
		{
			if( isset( $_GET[ 'cp1251' ] ) ) $content= iconv( 'CP1251', 'UTF-8//TRANSLIT//IGNORE', $content );
