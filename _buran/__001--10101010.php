				...
				if( ( stripos( $files, '/'. $tmp3 .'/' ) !== false || $files == '' ) && stripos( $ignore, '/'. $root_dir . $folder . $file .'/' ) === false )
				{
					...
					if( $tmp_filesize > 0 )
					{
						...
						for( $kk=0; $kk < @mysql_num_rows( $r_kws ); $kk++ )
						{
							...
							if( $regexp && @mysql_result( $r_kws, $kk, 'kw_1' ) == '-' && @mysql_result( $r_kws, $kk, 'kw_2' ) == '-' && @mysql_result( $r_kws, $kk, 'kw_3' ) == '-' && @mysql_result( $r_kws, $kk, 'kw_4' ) == '-' )
							{
								preg_match_all( $regexp, $r, $result );
								if( count( $result[ 0 ] ) >= 5 ) $tmp2= 'yea';
							}else{
