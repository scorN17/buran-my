...
?>
	<h1>Коды ответов и время загрузки сайтов</h1>
	
<?php

	$rr= mysql_query( "SELECT b3.* FROM {$b3} AS b3
		LEFT JOIN {$table1} ON c.id=b3.idc
			WHERE b3.code NOT LIKE 'ok%' OR b3.code_www NOT LIKE 'ok%' OR b3.`time` > 5 OR b3.time_www > 5
				ORDER BY b3.dt DESC" );
	...
	if( $rr && mysql_num_rows( $rr ) > 0 )
	{
		...
		while( $row= mysql_fetch_assoc( $rr ) )
		{
			...
			print '<tr>';
			print '<td>'. date( 'd.m.Y, H:i', $row[ 'dt' ] ) .'<br />'. $row[ 'code' ] .'<br />'. $row[ 'time' ] .'<br /><a target="_blank" href="http://'. $url .'/">'. $url .'</a></td>';
			print '<td>'. $row[ 'code_www' ] .'<br />'. $row[ 'time_www' ] .'<br /><a target="_blank" href="http://www.'. $url .'/">www.'. $url .'</a></td>';
			print '<td>';
