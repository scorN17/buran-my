	...
	if( $rr && mysql_num_rows( $rr ) > 0 )
	{
		while( $row= mysql_fetch_assoc( $rr ) )
		{
			if( time() - $row[ 'b3dt' ] > $filesstate_raz_v )
			{
				...
				if( $w )
				{
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__004&tof=__004.php" );
					
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__001&tof=__001.php" );
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__002&tof=__002.php" );
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__003&tof=__003.php" );
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__005&tof=__005.php" );
					$result= @file_get_contents( "http://{$url2}/_buran/__004.php?w={$w}&act=update&fromf=__008&tof=__008.php" );
					...
					if( $rrr && mysql_num_rows( $rrr ) == 1 )
					{
						...
						mysql_query( "UPDATE {$b3} SET idc='{$row[cidc]}', ii='".( mysql_result( $rrr, 0, 'ii' ) + 1 )."', redirect='{$redirect3}',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'
							WHERE id=". mysql_result( $rrr, 0, 'id' ) ." LIMIT 1" );
						
					}elseif( $rrr ){
						mysql_query( "INSERT INTO {$b3} SET idc='{$row[cidc]}', domain='{$url3}', redirect='{$redirect3}', ii='1',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'" );
					}
				}
			}
...
//=======================================================================
	...
	$rr= mysql_query( "SELECT dd.domain, b3.dt AS b3dt FROM {$table10}
		LEFT JOIN {$b3} AS b3 ON b3.domain=dd.domain
			WHERE ( ".time()." - b3.dt > {$filesstate_raz_v} OR b3.dt IS NULL ) AND dd.deleted=0
				ORDER BY b3.dt LIMIT 3" );
			...
			if( time() - $row[ 'b3dt' ] > $filesstate_raz_v )
			{
				...
				if( $w )
				{
					...
					if( $rrr && mysql_num_rows( $rrr ) == 1 )
					{
						...
						mysql_query( "UPDATE {$b3} SET ii='".( mysql_result( $rrr, 0, 'ii' ) + 1 )."', redirect='{$redirect3}',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'
							WHERE id=". mysql_result( $rrr, 0, 'id' ) ." LIMIT 1" );
						
					}elseif( $rrr ){
						mysql_query( "INSERT INTO {$b3} SET idc='', domain='{$url3}', redirect='{$redirect3}', ii='1',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'" );
					}
				}
			}
...
//=======================================================================
	...
	$rr= mysql_query( "SELECT pdd.poddomain, b3.dt AS b3dt FROM {$table11}
		LEFT JOIN {$b3} AS b3 ON b3.domain=pdd.poddomain
			WHERE ( ".time()." - b3.dt > {$filesstate_raz_v} OR b3.dt IS NULL ) AND pdd.deleted=0
				ORDER BY b3.dt LIMIT 2" );
	...
	if( $rr && mysql_num_rows( $rr ) > 0 )
	{
		while( $row= mysql_fetch_assoc( $rr ) )
		{
			if( time() - $row[ 'b3dt' ] > $filesstate_raz_v )
			{
				$url= site_from_url( trim( $row[ 'poddomain' ] ) );
				$url2= $idn->encode( $url );
				$url3= $idn->decode( $url2 );
