...
//=======================================================================
	...
	$rr= mysql_query( "SELECT c.id AS cidc, c.url, b3.dt AS b3dt FROM {$table1}
		...
		LEFT JOIN {$b3} AS b3 ON b3.idc=c.id
			WHERE ( r.`status`<>4 AND r.`status`<>8 AND r.`status`<>2 ) AND e.ntoday>0
			AND ( ".time()." - b3.dt > {$filesstate_raz_v} OR b3.dt IS NULL )
				ORDER BY b3.dt, c.id LIMIT 1" );
	...
	if( $rr && mysql_num_rows( $rr ) > 0 )
	{
		while( $row= mysql_fetch_assoc( $rr ) )
		{
			if( time() - $row[ 'b3dt' ] > $filesstate_raz_v )
			{
				...
				if( $w )
				{
					$result= @file_get_contents( "http://{$url2}/_buran/__001.php?act=antivirus&w={$w}&h={$dbh}&u={$dbu}&p={$dbp}&n={$dbn}" );
			...
			if( time() - $row[ 'b3dt' ] > $filesstate_raz_v )
			{
				...
				if( $w )
				{
					...
					if( $rrr && mysql_num_rows( $rrr ) == 1 )
					{
						...
						mysql_query( "UPDATE {$b3} SET ii='".( mysql_result( $rrr, 0, 'ii' ) + 1 )."',
							redirect='{$redirect3}',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'
							WHERE id=". mysql_result( $rrr, 0, 'id' ) ." LIMIT 1" );
						
					}elseif( $rrr ){
						mysql_query( "INSERT INTO {$b3} SET domain='{$url3}', ii='1',
							redirect='{$redirect3}',
							last='". mysql_escape_string( $result ) ."', dth='". date( 'Y-m-d-H-i-s' ) ."', dt='".time()."'" );
					}
				}
