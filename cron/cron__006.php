...
function main( $row )
{
	...
	$res= curl( $url2 );
	$res_www= curl( 'www.'. $url2 );
	$res= $res[ 'info' ];
	$res_www= $res_www[ 'info' ];
	
	$http_code= 'er';
	$http_code_www= 'er';
	$total_time= $res[ 'total_time' ];
	$total_time_www= $res_www[ 'total_time' ];
	
	if( $res[ 'http_code' ] == 200 || $res[ 'http_code' ] == 301 || $res[ 'http_code' ] == 302 )
		$http_code= 'ok';
	
	if( $res_www[ 'http_code' ] == 200 || $res_www[ 'http_code' ] == 301 || $res_www[ 'http_code' ] == 302 )
		$http_code_www= 'ok';
	
	$http_code .= $res[ 'http_code' ];
	$http_code_www .= $res_www[ 'http_code' ];
	...
	if( true )
	{
		...
		if( $rrr && mysql_num_rows( $rrr ) == 1 )
		{
			...
			mysql_query( "UPDATE {$b3} SET idc='{$row[cidc]}', ii='".( mysql_result( $rrr, 0, 'ii' ) + 1 )."',
				code='". mysql_escape_string( $http_code ) ."',
				code_www='". mysql_escape_string( $http_code_www ) ."',
				`time`='". mysql_escape_string( $total_time ) ."',
				time_www='". mysql_escape_string( $total_time_www ) ."',
